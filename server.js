...
async function api(req, res, url){
  try{
    ...
    if(req.method==='GET' && url.pathname==='/api/fit/prefill'){ const c=await db.getCustomer(url.searchParams.get('customer_id')); if(c&&!scopeOK(c.store)) return send(res,403,{ok:false,error:'다른 매장 손님이에요'}); const r=await db.fitPrefill(url.searchParams.get('customer_id'), url.searchParams.get('frame_db')); return send(res, r.ok?200:400, r); }
